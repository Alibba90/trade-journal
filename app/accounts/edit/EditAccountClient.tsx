...
export default function EditAccountClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        .from("accounts")
        .select(
          "account_number, firm, size, phase, balance, max_drawdown_percent, profit_target_percent"
        )
        .eq("id", id)
        .single();
      ...
      setForm({
        ...
        phase: (data.phase ?? "phase1") as "phase1" | "phase2" | "live",
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
      <form onSubmit={handleSave} className="w-full max-w-lg bg-white p-8 rounded-2xl shadow border">
