...
export default function TradesPage() {
  ...
  const [editOriginal, setEditOriginal] = useState<TradeRow | null>(null);

  const getAccountLabel = (a: any) => {
    const num = String(pick(a, ["account_num", "account_num_", "account_number", "number"], "") || "").trim();
    const firm = String(pick(a, ["firm", "company"], "") || "").trim();
    const size = toNum(pick(a, ["size", "account_size"], 0));
    const title = [num || "Счёт", firm ? `• ${firm}` : "", size ? `• $${fmtMoney(size)}` : ""]
      .join(" ")
      .replace(/\s+/g, " ")
      .trim();
    return title || "Счёт";
  ...
  function getBalanceFieldName(accRow: any): "balance" | "current_balance" | null {
    if (!accRow) return null;
    if (Object.prototype.hasOwnProperty.call(accRow, "balance")) return "balance";
    if (Object.prototype.hasOwnProperty.call(accRow, "current_balance")) return "current_balance";
    return null;
  }

  async function refreshLastTradeAt(accountId: string) {
    ...
    if (error) {
      console.log("refreshLastTradeAt ERROR:", error);
      return;
    }

    const lastDate = data && data.length ? (data[0] as any).trade_date : null;

    const { data: accRow } = await supabase.from("accounts").select("*").eq("id", accountId).maybeSingle();
    if (!accRow) return;

    if (Object.prototype.hasOwnProperty.call(accRow, "last_trade_at")) {
      const { error: updErr } = await supabase.from("accounts").update({ last_trade_at: lastDate }).eq("id", accountId);
      if (updErr) console.log("UPDATE last_trade_at ERROR:", updErr);
    }
  }

  async function applyAccountDelta(accountId: string, deltaPnl: number, maybeTradeDate?: string | null) {
    const { data: accRow, error } = await supabase.from("accounts").select("*").eq("id", accountId).maybeSingle();
    if (error) {
      console.log("applyAccountDelta account fetch ERROR:", error);
      return;
    }
    if (!accRow) return;

    const field = getBalanceFieldName(accRow);
    if (field) {
      const cur = toNum(accRow[field]);
      const next = cur + toNum(deltaPnl);

      const payload: any = { [field]: next };

      if (Object.prototype.hasOwnProperty.call(accRow, "last_trade_at") && maybeTradeDate) {
        const prev = accRow.last_trade_at ? String(accRow.last_trade_at) : null;
        if (!prev || String(maybeTradeDate) > prev) payload.last_trade_at = maybeTradeDate;
      }

      const { error: updErr } = await supabase.from("accounts").update(payload).eq("id", accountId);
      if (updErr) console.log("applyAccountDelta UPDATE ERROR:", updErr);
    }

    if (!maybeTradeDate) {
      await refreshLastTradeAt(accountId);
    }
  }
  ...
  async function reload() {
    ...
    if (accErr) console.log("ACCOUNTS ERROR:", accErr);
    ...
    if (!form.account_id && accArr.length) {
      setForm((p) => ({ ...p, account_id: accArr[0].id }));
    }
  ...
  async function createTrade() {
    ...
    if (error) {
      console.log("INSERT TRADE ERROR:", error);
      setErrMsg(error.message);
      return;
    }

    await applyAccountDelta(form.account_id, pnl, form.trade_date);
  ...
  // ✅ то же самое для редактирования: при смене outcome нормализуем сумму
  ...
  async function saveEdit(id: string) {
    ...
    if (!edit.account_id) return setErrMsg("В редактировании: выбери счёт.");
    ...
    if (!String(edit.setup || "").trim()) return setErrMsg("В редактировании: укажи сетап.");
    ...
    if (editOriginal) {
      const oldAcc = editOriginal.account_id;
      const newAcc = String(edit.account_id);

      const oldPnl = toNum(editOriginal.pnl_money);
      const newPnl2 = newPnl;

      const oldDate = String(editOriginal.trade_date);
      const newDate = String(edit.trade_date);

      if (oldAcc === newAcc) {
        const delta = newPnl2 - oldPnl;
        if (delta !== 0) await applyAccountDelta(newAcc, delta, newDate);
        if (oldDate !== newDate) await refreshLastTradeAt(newAcc);
      } else {
        await applyAccountDelta(oldAcc, -oldPnl, null);
        await applyAccountDelta(newAcc, newPnl2, newDate);
      }
    }
  ...
  async function deleteTrade(id: string) {
    ...
    const { data: row, error: getErr } = await supabase
      .from("trades")
      .select("id,account_id,pnl_money,trade_date")
      .eq("id", id)
      .maybeSingle();

    if (getErr) {
      console.log("GET TRADE BEFORE DELETE ERROR:", getErr);
      setErrMsg(getErr.message);
      return;
    }

    const accId = row?.account_id as string | undefined;
    const pnl = toNum(row?.pnl_money);
    ...
    if (accId) {
      await applyAccountDelta(accId, -pnl, null);
    }
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-sm border">
          <h1 className="text-2xl font-bold text-gray-900">Журнал сделок</h1>
          <p className="mt-2 text-gray-600">Войди в аккаунт, чтобы видеть и добавлять сделки.</p>
          <Link
            href="/login"
    ...
    <main className="min-h-screen bg-gray-50">
      <header className="mx-auto max-w-6xl px-6 pt-10 pb-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Журнал сделок</h1>
            <p className="mt-2 text-sm text-gray-600">
              Стоп/БУ- автоматически делает сумму отрицательной. Тэйк/БУ+ — положительной.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-10">
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm border">
          <div className="flex items-center justify-between gap-3">
            <div className="text-lg font-semibold text-gray-900">Добавить сделку</div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            {/* ✅ СЕТАП */}
            <Field label="Сетап (обязательно)">
              <input
                className="w-full rounded-xl border px-3 py-2"
                placeholder="например: SFP + Имбаланс"
                value={form.setup}
            ...
            <Field label="Киллзона">
              <select
                className="w-full rounded-xl border px-3 py-2"
                value={form.killzone}
                onChange={(e) => setField("killzone", e.target.value)}
              >
                {KILLZONES.map((k) => (
            ...
            <Field label="Фаза рынка">
              <select
                className="w-full rounded-xl border px-3 py-2"
                value={form.market_phase}
                onChange={(e) => setField("market_phase", e.target.value)}
              >
                {MARKET_PHASES.map((m) => (
            ...
            <Field label="Скрин ХТФ (URL)">
              <input
                className="w-full rounded-xl border px-3 py-2"
                placeholder="https://..."
                value={form.htf_screenshot_url}
                onChange={(e) => setField("htf_screenshot_url", e.target.value)}
              />
            </Field>
            ...
            <Field label="Скрин ЛТФ (URL)">
              <input
                className="w-full rounded-xl border px-3 py-2"
                placeholder="https://..."
                value={form.ltf_screenshot_url}
                onChange={(e) => setField("ltf_screenshot_url", e.target.value)}
              />
            </Field>
          </div>
          ...
          <div className="mt-3">
            ...
            <textarea
              ...
              placeholder="Контекст, мысли, ошибки…"
              value={form.comment}
        ...
        <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm border">
          <div className="flex items-center justify-between gap-3">
            <div className="text-lg font-semibold text-gray-900">Сделки ({trades.length})</div>
            <button
              onClick={reload}
          ...
          {trades.length === 0 ? (
            ...
            <div className="mt-4 space-y-3">
              {trades.map((t) => {
                ...
                const accLabel = a ? getAccountLabel(a) : "Счёт";
                      ...
                      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                        <div>
                          ...
                          <div className="mt-1 text-sm text-gray-800">
                            <span className="text-gray-500">Сетап:</span>{" "}
                            <span className="font-semibold">{t.setup || "-"}</span>
                          </div>

                          <div className="mt-1 text-xs text-gray-500">
                            {accLabel} • {kzRu} • {dirRu} • {mpRu} • Риск {toNum(t.risk_pct).toFixed(2)}% • RR{" "}
                            {toNum(t.rr).toFixed(2)} • {outRu}
                          </div>
                          ...
                          {t.comment ? (
                            <div className="mt-2 text-sm text-gray-700 whitespace-pre-wrap">{t.comment}</div>
                          ) : null}
                        </div>
                      ...
                      <>
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                          ...
                          <Field label="Сетап (обязательно)">
                            <input
                              className="w-full rounded-xl border px-3 py-2"
                              value={edit.setup}
                              onChange={(e) => setEdit((p: any) => ({ ...p, setup: e.target.value }))}
                            />
                          </Field>
                          ...
                          <Field label="Результат">
                            <select
                              className="w-full rounded-xl border px-3 py-2"
                              value={edit.outcome}
                              onChange={(e) => onEditOutcomeChange(e.target.value)}
                            >
                              {OUTCOMES.map((o) => (
                                <option key={o.v} value={o.v}>
                                  {o.ru}
                          ...
                          <Field label="Результат $ (просто число)">
                            <input
                              className="w-full rounded-xl border px-3 py-2"
                              value={edit.pnl_money}
                              onChange={(e) => setEdit((p: any) => ({ ...p, pnl_money: e.target.value }))}
                            />
                          </Field>
                        </div>
                        ...
                        <div className="mt-3">
                          ...
                          <textarea
                            className="w-full rounded-xl border px-3 py-2 min-h-[90px]"
                            value={edit.comment}
                            onChange={(e) => setEdit((p: any) => ({ ...p, comment: e.target.value }))}
                          />
                        </div>
                      </>
                    )}
